...
export function CartSummary({ itemCount, subtotalCents, savingsCents }: CartSummaryProps) {
  ...
  return (
    <div className="shadow-e1 flex flex-col gap-4 rounded-2xl border border-border bg-surface p-5">
      <h2 className="text-h4 text-text-primary">Order Summary</h2>
      ...
      <Link
        href="/checkout"
        className="text-small-semibold shadow-e2 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-danger to-danger/90 text-white transition-transform hover:scale-[1.02] hover:shadow-e3 active:scale-[0.99]"
      >
        Proceed to Checkout
      </Link>
